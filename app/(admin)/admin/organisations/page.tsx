...
export default function AdminOrganisationsPage() {
  ...
  async function handleTierChange(orgId: string, newTier: string) {
    ...
    if (res.ok) setOrgs((prev) => prev.map((o) => o.id === orgId ? { ...o, tier: newTier } : o));
  ...
  const columns: ColumnDef<AdminOrg>[] = [
    {
      ...
      render: (org) => (
        <div className="flex items-center gap-2 lg:gap-3">
          ...
          <span className="text-xs font-black tracking-tight" style={{ color: "var(--text-primary)" }}>{org.name}</span>
        </div>
      ),
    },
    {
    ...
    {
      key: "actions", header: "Tier Control", align: "right",
      render: (org) => (
