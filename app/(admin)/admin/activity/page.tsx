...
import { Activity, UserPlus, Upload, TrendingUp, Zap, Flame, Clock, Calendar } from "lucide-react";
...
export default function AdminActivityPage() {
    ...
    return (
      <div className="space-y-4 lg:space-y-6 animate-pulse">
        ...
        {Array.from({ length: 5 }).map((_, i) => (
          <div key={i} className="premium-card p-3 lg:p-4 flex gap-3 lg:gap-4">
            <div className="w-9 h-9 lg:w-10 lg:h-10 rounded-xl" style={{ background: "var(--bg-inset)" }} />
  ...
  return (
    <div className="space-y-4 lg:space-y-6 animate-scale-in">
      ...
      {/* Filters */}
      <div className="flex items-center gap-1 p-1.5 rounded-xl w-fit overflow-x-auto"
      ...
      {/* Timeline */}
      {filtered.length === 0 ? (
        <div className="premium-card p-10 lg:p-12 text-center" style={{ boxShadow: "var(--shadow-inset)" }}>
          ...
          {Object.entries(grouped).map(([date, items]) => (
            <div key={date}>
              ...
              <div className="flex items-center gap-2 lg:gap-3 mb-3 lg:mb-4">
                <Calendar className="w-3.5 h-3.5 shrink-0" style={{ color: "var(--text-muted)", opacity: 0.4 }} />
                <span className="text-[9px] lg:text-[10px] font-black uppercase tracking-[0.2em] text-text-muted opacity-40 truncate">
                  {date}
                </span>
              ...
              <div className="space-y-2 pl-1 lg:pl-2">
                {items.map((a) => {
                  ...
                  return (
                    <div key={a.id}
                      className="flex items-center gap-3 lg:gap-4 p-3 lg:p-4 rounded-xl group transition-all duration-300 relative"
                      style={{ background: "var(--neu-base)", border: "var(--card-border)" }}>
                      {/* Timeline connector dot */}
                      <div className="absolute left-0 top-1/2 -translate-y-1/2 -translate-x-1/2 w-3 h-3 rounded-full border-2"
                        style={{ background: "var(--bg-base)", borderColor: cfg.color }} />
                      {/* Icon */}
                      <div className="w-9 h-9 lg:w-10 lg:h-10 rounded-xl flex items-center justify-center shrink-0 transition-transform group-hover:scale-110"
                      ...
                      <div className="flex-1 min-w-0">
                        ...
                        <p className="text-xs font-bold truncate" style={{ color: "var(--text-primary)" }}>{a.description}</p>
                        {a.org_name && (
                          <p className="text-[9px] font-bold uppercase tracking-widest text-text-muted opacity-40 truncate">{a.org_name}</p>
                        )}
                      </div>
                      {/* Time */}
                      <span className="text-[10px] font-bold text-text-muted opacity-30 shrink-0 tabular-nums">{time}</span>
                    </div>
                  );
                })}
              </div>
