...
import { UserPlus, Trash2, Crown, User, Clock, Check, X, ShieldAlert, Building2, AlertTriangle, ChevronDown } from "lucide-react";
...
export default function TeamPage() {
        ...
        return (
          <div className="flex items-center gap-4">
            ...
            <div>
              ...
              <p className="text-[8px] font-bold text-text-muted mt-1 opacity-40 uppercase tracking-widest">
    ...
    },
    {
      ...
      render: (member) => (
        <div className="inline-flex items-center gap-3">
          {member.role === "owner" ? (
            <span className="text-[9px] px-3 py-2 rounded-lg font-black uppercase tracking-widest bg-yellow-500/10 text-yellow-700 border border-yellow-500/20">
              System Owner
            </span>
          ) : (
            <div className="relative inline-block text-left">
              {isAdmin ? (
                <>
                  <button
                    type="button"
                    onClick={() => setOpenRoleMenuUserId(openRoleMenuUserId === member.id ? null : member.id)}
                    className="flex items-center gap-2 text-[10px] font-black uppercase tracking-[0.1em] pl-4 pr-3 py-2.5 rounded-xl bg-white border border-gray-100 text-gray-900 hover:border-gt-green-400 shadow-sm transition-all outline-none cursor-pointer"
                  >
                    <span>{normalizeRole(member.role)}</span>
                    <ChevronDown className="w-3.5 h-3.5 text-text-muted" />
                  </button>
                  {openRoleMenuUserId === member.id && (
                    <div className="absolute top-full right-0 mt-2 z-50 w-36">
                      <div
                        className="animate-scale-in border border-border-default/50 shadow-[0_20px_50px_rgba(0,0,0,0.12)] p-3 rounded-[2rem] flex flex-col gap-1 text-left"
                      ...
                      >
                        {["owner", "member"].map((r) => (
                          <button
                            ...
                            className={`w-full text-left px-5 py-3 rounded-full text-[9px] font-black uppercase tracking-widest transition-all ${
                              normalizeRole(member.role) === r
                            ...
                            style={normalizeRole(member.role) !== r ? { background: "transparent" } : {}}
                          >
                            {r}
                          </button>
                        ))}
          ...
          {isAdmin && member.role !== "owner" && (
            <Button
              ...
              className="w-9 h-9 !p-0 shadow-sm border border-red-100 hover:scale-105 active:scale-95 transition-all flex items-center justify-center shrink-0"
            >
  ...
  return (
    ...
    >
      {/* Role Menu Backdrop */}
      {openRoleMenuUserId && (
        <div className="fixed inset-0 z-40" onClick={() => setOpenRoleMenuUserId(null)} />
      )}
        ...
        {isAdmin && (
          <div className="lg:col-span-1 space-y-4 sm:space-y-6">
            <div className="premium-card p-5 sm:p-8 space-y-5 sm:space-y-6">
              ...
              {seatsFull ? (
                <div className="p-8 rounded-3xl border border-border-default bg-white shadow-sm space-y-6 flex flex-col items-center justify-center text-center">
                  <div className="space-y-2">
                    <p className="text-[11px] font-black text-text-primary uppercase tracking-widest">
                      Capacity Limit Reached
                    </p>
                    <p className="text-[10px] text-text-muted">
                      Upgrade your subscription to invite more auditors.
                    </p>
                  </div>
                  <Button
                    variant="primary"
                          ...
                          {isAdmin ? (
                            <>
                              <button
                                type="button"
                                onClick={() => setOpenRoleMenuUserId(openRoleMenuUserId === member.id ? null : member.id)}
                                className="flex items-center gap-1 text-[9px] font-black uppercase tracking-[0.1em] pl-3 pr-2 py-2 rounded-xl bg-white border border-gray-100 text-gray-900 hover:border-gt-green-400 shadow-sm transition-all outline-none cursor-pointer"
                              >
                                <span>{normalizeRole(member.role)}</span>
                                <ChevronDown className="w-3 h-3 text-text-muted" />
                              </button>
                              {openRoleMenuUserId === member.id && (
                                <div className="absolute top-full right-0 mt-2 z-50 w-32">
                                  <div
                                    className="animate-scale-in border border-border-default/50 shadow-[0_20px_50px_rgba(0,0,0,0.12)] p-2.5 rounded-[2rem] flex flex-col gap-1 text-left"
                                    style={{ background: "var(--bg-elevated)" }}
                                  >
                                    {["owner", "member"].map((r) => (
                                      <button
                                        ...
                                        className={`w-full text-left px-4 py-2.5 rounded-full text-[8px] font-black uppercase tracking-widest transition-all ${
                                          normalizeRole(member.role) === r
                                            ? "bg-gt-green-500 text-white shadow-md shadow-gt-green-500/10"
                                            : "text-text-primary hover:bg-bg-inset/50 hover:text-text-primary"
                                        }`}
